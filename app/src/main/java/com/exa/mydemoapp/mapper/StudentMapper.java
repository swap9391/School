...
public class StudentMapper extends DbMapper<StudentModel> {
    ...
    protected Collection<StudentModel> doMap(Cursor rs) {
        ...
        while (rs.moveToNext()) {
            ...
            bean.setSchoolName(getString(rs, 2));
            bean.setDateStamp(getString(rs, 3));
            ...
            bean.setStudentBloodGrp(getString(rs, 11));
            bean.setGender(getString(rs, 12));
            bean.setTotalFees(getInt(rs, 13));
            bean.setInstallmentType(getString(rs, 14));
            bean.setInstallment1(getString(rs, 15));
            bean.setInstallment2(getString(rs, 16));
            bean.setInstallment3(getString(rs, 17));
            bean.setRollNumber(getString(rs, 18));
            bean.setContactNumber(getString(rs, 19));
            bean.setDateInsvestment2(getString(rs, 20));
            bean.setDateInsvestment3(getString(rs, 21));
            bean.setSubscribed(getString(rs, 22));
            bean.setVisiblity(getString(rs, 23));
            lst.add(bean);
        }
        return lst;
    }
